...
public class ProductApi {
    ...
    public ResponseEntity<?> findAllList(@RequestParam(value = "idCategory", required = false) Long idCategory){
        List<Product> result = productService.findAll(idCategory);
    ...
    @GetMapping("/public/san-pham-ban-chay-page")
    public ResponseEntity<?> sanPhamBanChayPage(Pageable pageable){
        Page<Product> result = productService.sanPhamBanChay(pageable);
        return new ResponseEntity<>(result,HttpStatus.OK);
    }

    @GetMapping("/public/san-pham-moi")
    public ResponseEntity<?> sanPhamMoi(Pageable pageable){
        Page<Product> result = productService.sanPhamMoi(pageable);
        return new ResponseEntity<>(result,HttpStatus.OK);
    }
    ...
    @GetMapping("/public/sanPhamKhuyenMai")
    public ResponseEntity<?> sanPhamKhuyenMai(Pageable pageable){
        Page<Product> result = productService.sanPhamKhuyenMai(pageable);
        return new ResponseEntity<>(result,HttpStatus.OK);
    }

    @GetMapping("/admin/san-pham-ban-theo-ngay")
    public ResponseEntity<?> sanPhamBanTheoNgay(@RequestParam(value = "date", required = false) Date date){
        List<ProductSell> result = productService.sanPhamBanTheoNgay(date);
        return new ResponseEntity<>(result,HttpStatus.OK);
    }

    @GetMapping("/admin/san-pham-ban-theo-thang")
    public ResponseEntity<?> sanPhamBanTheoThang(@RequestParam Integer month, @RequestParam Integer year){
        List<ProductSell> result = productService.sanPhamBanTheoThang(month, year);
        return new ResponseEntity<>(result,HttpStatus.OK);
    }
}
