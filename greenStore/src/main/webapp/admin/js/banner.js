...
async function loadBanner() {
    ...
    var url = 'http://localhost:8080/api/banner/public/search'
...
async function saveBanner() {
    ...
    // Validation for page name and product ID
    if (!pageName) {
        toastr.warning("Tên trang không được để trống.");
        document.getElementById("loading").style.display = 'none';
        return;
    }

    if (!idProduct) {
        toastr.warning("ID sản phẩm không được để trống.");
        document.getElementById("loading").style.display = 'none';
        return;
    }
    if (/[^0-9]/.test(idProduct)) {
        toastr.warning("ID sản phẩm không được chứa chữ cái.");
        document.getElementById("loading").style.display = 'none';
        return;
    }
